...
1. 先生成 React Server Component Payload (RSC Payload)，没有 Client Component，为他们留了一些嵌入位置。另外注意数据实际上是分离的，方便做单独缓存。是用 RSC 的原因是因为当我们在 Server Component 重新 fetch 数据时，嵌在 Server Component 里的 Client Component 的状态不会被重置，因为其用的是 RSC，而不是 HTML。
