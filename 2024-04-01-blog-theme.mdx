...
tags:
  - 'Next.js'
  - 'React.js'
abstract: '可能你已经注意到了，点击 Header 的 Cotton Candy 可以更换主题，同时它会使用 LocalStorage 记住。如果我们想用 Next.js 做一个主题切换，同时能够记住，事情就会变得有点复杂，这将有关 Next.js 的 Render 方式，让我们一起探索一下。'
---
...
1. 先生成 React Server Component Payload(RSC Payload)，没有 Client Component，为他们留了一些嵌入位置。另外注意数据实际上是分离的，方便做单独缓存。
2. 然后用 RSC 和 Client Component 的**指导**生成 HTML。（这里的指导时原文的 instructions，很奇怪对吧，不是嵌进去一起渲染，而是“指导”）

然后到了客户端，Next.js 会把 HTML, RSC, Client Component 发给客户端。HTML 然后客户端会用剩下来的两个生成可交互的部分，而**嵌入**的方式是使用 [hydrate](https://react.dev/reference/react-dom/client/hydrateRoot)，实际上就是绑定一些事件监听器。
...
那也就是说，Server 生成的 HTML 模版，会用于首次预览。那客户端拿到同样的 JS 之后会异步执行，并添加一些事件操作。这时候会有一些重复的内容，特别是一开始的默认值，也就是 State 的 init 的值。如果我们继续使用上述的代码构建，在 Component 外面定一个一个随机数，其会在 SSG 的时候执行一次（并且是个确定数），生成对应的 HTML。随后再在客户端执行一次，会和 SSG 的时候生成的不一样。在这种情况下，Next.js 的逻辑是**首次渲染**以 SSG 生成的 HTML 为准，所以当我们尝试刷新页面时，你看不到任何变化，控制台里则有 [Text content does not match server-rendered HTML](https://nextjs.org/docs/messages/react-hydration-error) 的错误。

我们实际上有两种方案解决这个问题，使用 [next-themes](https://github.com/pacocoursey/next-themes)，其原理是注入一个 script 块到 HTML 模版中，保证其在 DOM render 完成之前执行，使用它会导致上述的 Text content does not match server-rendered HTML 问题，那么作者给出的方案是直接在根 html 标签上添加 suppressHydrationWarning 属性忽视掉。第二个方案是使用 `useEffect` hook，确保**首次渲染**完成后更改主题，这种方案有个明显的缺陷，那就是很显然会出现闪烁。
...
Client Component hydrate 的位置问题，使用 Page Router 时，Client Component 会直接在 Server 上 Hydrate。而使用 App Router 时会在 Client Hydrate，以减少初次 HTML 的大小。因为这个问题导致其渲染了“两次”。
...
Server Component 的引入主要是为了解决服务器[数据预注入](https://www.joshwcomeau.com/react/server-components/#introduction-to-react-server-components-3)的问题，当它与 Client Component 各种嵌套后，Server 为了生成 HTML，需要先执行一遍 Client Component，包括其 `useState` 的内容，但不包含 `useEffect` 的内容。虽然说是嵌套，但由于无法在 Client Component 中 import Server Component，因此不会将 Props 传给它，不然传给它的是一个 State，那会影响 Server Component 的执行（因为他执行一遍）。但是我们用了一些奇怪的技巧来传递 State，也就是使用 Context，这样就可以在两个 Client Component 中间传递状态，越过了那些作为 children 的 Server Component。

为了渲染 Server Component，那就得先跑嵌套它的 Client Component，同时 Client Component 由于包含状态，因此会到 Client 再来一遍，执行两次就来了，那么由于 Server Component 只会**执行一次**，对于 SSG 其只会在 build 时执行一次，那么出现不同时就会以 Server 上生成的为准，而不是 hydrate 之后的为准（当然这实际上是 Next 不期望发生的）。

## 怎么解决？

就像我上面说的那样，一切涉及到用本地数据 init State 的，包括我之前的讨论 Tags 时[说到的](/posts/2024-03-29-blog-tags)，这个问题暂时可以先放在 `useEffect` 中，或者使用 hack 的方法注入 `script` 的 tag。最后这种方法我觉得只是临时的解决方案，因为这个问题应该会改进，毕竟 toggle 主题实际上非常常用。而改进的方案应该将这种 init State 的操作延后到客户端执行，且需要在 DOM 之前。

## 状态究竟应该保存在哪？

我觉得这是个永恒的问题，实际上面还有一种解决方案，就是使用 cookie 确保每次 SSR 都是正确的结果就行了，但这会导致 Next 认为整个页面都是 dynamic 的，导致其余的东西也会 dynamic SSR，这部分是值得继续优化的。
